...
function genFragment(
    ...
    if (
        nodeName === 'customcodeblock'
    ) {
        const entityConfig = {};
        entityConfig.language = node.getAttribute ? node.getAttribute('language') || node.language : node.language;
        entityConfig.value = node.getAttribute ? node.getAttribute('value') || node.value : node.value;
        entityConfig.html = node.innerHTML;
        const entityId = Entity.__create(
            'CUSTOMCODE',
            'MUTABLE',
            entityConfig,
