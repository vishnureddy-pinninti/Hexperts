...
export default function processInlineTag(
    ...
    if (styleToCheck) {
        ...
        inlineStyle = inlineStyle.withMutations((style) => {
            const { color } = htmlElement.style;
            const { backgroundColor } = htmlElement.style;
            const { fontSize } = htmlElement.style;
            const fontFamily = htmlElement.style.fontFamily.replace(/^"|"$/g, '');
            const { fontWeight } = htmlElement.style;
            const { textDecoration } = htmlElement.style;
            const { fontStyle } = htmlElement.style;
            if (color) {
